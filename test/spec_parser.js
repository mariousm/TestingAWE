...
var aut = require("../dist/aut/automation");
...
describe("UnitTest PARSER", function () {
    it.only("writeFile CORRECT", function () {
        for (var i = 0; i < 2; ++i) {
            var name_1 = "writeFile" + (i + 1).toString() + ".ts";
            var file = path.join(__dirname, "materialOUT", name_1);
            // Comprobamos si existe el fichero, si existe lo borramos
            remove(file);
            // Creamos el fichero
            parser.writeFile(file, name_1);
            // Comprobamos que lo ha creado
            assert.equal(exist(file), true);
            // Comprobamos que el contenido del fichero es el esperado
            var content = fs.readFileSync(file, 'utf8');
            assert.equal(content, name_1);
        }
    });
    ...
    // });
    it.only("htmlToString CORRECT", function () {
        for (var i = 0; i < 2; ++i) {
            // INICIAL
            var name_2 = "html" + (i + 1).toString() + ".html";
            var file = path.join(__dirname, "materialIN", name_2);
            var html = parser.htmlToString(file);
            // ESPERADO
            name_2 = "html" + (i + 1).toString() + ".txt";
            file = path.join(__dirname, "materialIN", name_2);
    ...
    // })
    it.only("parser CORRECT", function () {
        ...
        var file = path.join(__dirname, "materialIN", "html1.html");
        var expected = [["h3", "dashboard_title"], ["div", "dashboard_div"], ["a", "dashboard_a_details_hero_{{i}}"], ["div", "dashboard_div_detail_hero_{{i}}"], ["h4", "dashboard_h4_detail_hero_name_{{i}}"]];
        var elements = parser.parser(file);
    ...
    // });
    it.only("nameTagClass CORRECT", function () {
    ...
    //     assert.equal(name, '');
    // });
    it.only("nameVarFunc CORRECT", function () {
        ...
        var name = parser.nameVarFunc("dashboard_title");
    ...
    // })
    it.only("paramsIdTag CORRECT", function () {
        ...
        var params = parser.paramsIdTag("dashboard_a_details_hero_{{i}}");
    ...
    // });
    it.only("createFileApplicationInstance CORRECT", function () {
        // Comprobamos que pasándole la ruta de la aplicación y la ruta de salida nos genera el fichero
        var inPath = path.join(__dirname, "..");
        var outPath = path.join(__dirname, "materialOUT");
        // Comprobamos si existe el fichero, si existe lo borramos
        ...
        var content = fs.readFileSync(path.join(outPath, "applicationInstance.awe.ts"), 'utf8');
        var expected = "import { ApplicationInstance } from \"testing-awe\";\n\nexport const applicationInstance: ApplicationInstance = ApplicationInstance.getInstance(\"testing-awe\");";
